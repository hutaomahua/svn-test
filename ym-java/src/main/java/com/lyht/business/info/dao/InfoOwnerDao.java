...
public interface InfoOwnerDao extends JpaRepository<InfoOwnerEntity, Integer> {
	
	
	@Query(value = "select * from t_info_owner where LENGTH(id_card) = 15",nativeQuery = true)
	List<InfoOwnerEntity> findIdCardLength15();
	
	@Query(value = "select * from t_info_owner where LENGTH(id_card) = 18",nativeQuery = true)
	List<InfoOwnerEntity> findIdCardLength18();
	...
	@Query(value = "select * from t_info_owner",nativeQuery = true)
	List<InfoOwnerEntity> getAll();
	...
	public InfoOwnerDetailVO findOneByNm(@Param("nm") String nm);
	...
	@Query(value = "SELECT " + " tt.nm AS ownerNm, tt.name AS ownerName, tt.is_satisfy AS isSatisfy,"
			...
			+ " pdv4.name AS placeType, tt.place_name AS placeName," + " tt.place_address AS placeAddress,"
			+ " tt.id_card AS idCard," + " pdv1.name AS zblx, pdv2.name AS nature," + " pdv3.name AS scopeName,"
	...
	@Query(value = "SELECT " + " pr.name AS regionName, pr.city_code AS cityCode, pr.parent_code AS parentCode,"
			...
			+ " SELECT IF(SUM(i_population) > 0, SUM(i_population), 0) FROM t_info_owner tif WHERE tif.region = pr.city_code ) AS population,"
			+ " (" + " SELECT COUNT(1) FROM t_info_enterprise tie" + " WHERE tie.region = pr.city_code"
			+ " ) AS enterNumber," + " (" + " SELECT IFNULL(SUM(tih.area),0) FROM t_info_houses tih"
			...
			+ " SELECT IFNULL(SUM(tit.num),0) FROM t_info_trees tit" + " WHERE tit.region = pr.city_code"
	...
	@Query(value = "SELECT " + " tt.id AS id, tt.nm AS nm, tt.project_nm AS projectNm,"
			...
			+ " tt.xiang AS xiang, tt.zu AS zu,"
			+ " tt.place_type AS placeType, tt.place_name AS placeName, tt.to_where AS toWhere,"
			+ " tt.place_address AS placeAddress,"
			+ " tt.is_satisfy AS isSatisfy, tt.define AS define, tt.fh_nm AS fhNm,"
			+ " tt.gs_state AS gsState, tt.status AS status,"
			...
			+ " pdv7.name AS natureValue, pdv8.name AS ownerTypeValue, pdv9.name AS placeTypeValue"
			+ " FROM t_info_owner tt" + " LEFT JOIN pub_region tpr ON tt.region = tpr.city_code"
			...
			+ " LEFT JOIN pub_dict_value pdv8 ON tt.owner_type = pdv8.nm"
			+ " LEFT JOIN pub_dict_value pdv9 ON tt.place_type = pdv9.nm" + " WHERE 1=1"
			...
			+ " AND IF (:nm is not null, tt.nm = :nm, 1=1)"
			+ " AND IF (:placeType is not null, tt.place_type = :placeType, 1=1)", countQuery = " SELECT" + " COUNT(1)"
					+ " FROM t_info_owner tt" + " LEFT JOIN pub_region tpr ON tt.region = tpr.city_code"
					+ " LEFT JOIN pub_project tpp ON tt.project_nm = tpp.id"
					+ " LEFT JOIN pub_dict_value pdv1 ON tt.zblx = pdv1.nm"
					+ " LEFT JOIN pub_dict_value pdv2 ON tt.stage = pdv2.nm"
					...
					+ " AND IF (:isDirtyData IS NOT NULL, IF(:isDirtyData = '0', tt.id_card IS NULL, tt.id_card IS NOT NULL), 1=1)"
					+ " AND IF (:nm is not null, tt.nm = :nm, 1=1)"
					+ " AND IF (:placeType is not null, tt.place_type = :placeType, 1=1)", nativeQuery = true)
	Page<InfoOwnerVO> page(@Param("region") String region, @Param("scope") String scope, @Param("name") String name,
			...
			@Param("placeType") String placeType, Pageable pageable);
	...
	@Modifying
	@Query(value = "update t_info_owner set age = :age where id = :id",nativeQuery = true)
