...
public interface InfoBuildingDao extends JpaRepository<InfoBuildingEntity, Integer> {
	...
	@Query(value = "SELECT " 
			...
			+ " LEFT JOIN pub_files file ON file.table_name = 't_info_building' AND file.table_pk_column = tt.nm "
			+ " WHERE 1=1"
			...
			, countQuery = " SELECT"
					...
					+ " AND IF (:ownerNm is not null, tt.owner_nm = :ownerNm, 1=1)"
					, nativeQuery = true)
	Page<InfoBuildingVO> page(@Param("region") String region,
