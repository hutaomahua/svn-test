...
public class CommonUtil {
	 ...
	 * 
	 * @param s 日期时间型的字符串
	 * @return 若参数s为null或为"", 返回null; 若参数s的格式不能对应"yyyy-MM-dd HH:mm:ss"或不是一个合法的日期,
	 *         返回null; 否则返回字符串对应的日期;
	 */
	...
	public static boolean isLetterOrDigit(String s) {
		...
		for (int i = 0; i < chars.length; i++) {
			if (!Character.isDigit(chars[i]) && !Character.isUpperCase(chars[i]) && !Character.isLowerCase(chars[i])) {
				result = false;
				break;
			}
		}
	...
	public static List<Integer> parseIntegerList(String params) {
		List<String> asList = Arrays.asList(params.split(","));
	...
	public static boolean isZeroOrNull(BigDecimal val) {
		...
		if (0 == val.compareTo(BigDecimal.ZERO)) {
			return true;
		}
		return false;
	}
